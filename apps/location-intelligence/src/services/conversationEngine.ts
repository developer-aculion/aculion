...
export function generateLocationExplanation(analytics: LocationAnalytics | null): string {
  ...
  const points: string[] = [];
  if (f.bank_count > 3) points.push("High banking density");
  if (f.shopping_count > 3) points.push("Strong shopping activity");
  if (f.metro_count > 0) points.push("Excellent metro connectivity");
  if (f.residential_density > 50) points.push("High residential population");
  if (f.commercial_density > 40) points.push("Strong commercial ecosystem");
  if (f.bus_count > 5) points.push("Convenient bus transit network");
  if (f.road_density > 10) points.push("Heavy vehicular flow corridors");
  ...
  switch (intent) {
    ...
    case "AUDIENCE_PREDICTION": {
      ...
      return `### Audience Demographic Prediction: ${areaName}\n` +
        ...
        `- **Traffic Driver**: Proximity to public transit (${f!.bus_count} bus stops, ${f!.metro_count} metro stations)\n` +
    ...
    case "FORMAT_SUGGESTION": {
      ...
      if (k.overall_score > 80 && f!.transit_accessibility > 80) {
      ...
      } else if (f!.commercial_density > 60) {
      ...
      return `### Billboard Campaign Format Recommendation\n` +
        ...
        `_This recommendation fits the local commercial activity index (${f!.commercial_density}%) and transit connectivity._`;
    }
    ...
    case "ROI_ESTIMATION": {
      ...
      const frequency = f!.transit_accessibility > 75 ? "High" : "Moderate";
    ...
    case "MARKETING_INSIGHTS": {
      ...
      return `### Spatial Marketing Consultant Insights: ${areaName}\n` +
        ...
        `3. **Dwell Time leverage**: High transit connectivity (${f!.transit_accessibility}/100) allows detailed product storytelling as pedestrians wait near transit terminals.`;
    }
    ...
    case "FIND_NEARBY_POIS": {
      ...
      return `### Points of Interest (POIs) near ${areaName}\n` +
        `Within a **${analytics.radius}m** radius, our spatial database has mapped a total of **${f.total_pois} POIs**:\n` +
        `- **Bank & ATM branches**: ${f.bank_count}\n` +
        `- **Retail & Shopping centers**: ${f.shopping_count}\n` +
        `- **Food & Dining outlets**: ${f.restaurant_count}\n` +
        `- **Office & IT complexes**: ${f.office_count}\n` +
        `- **Transit stops (Bus/Metro)**: ${f.bus_count + f.metro_count}`;
    }
