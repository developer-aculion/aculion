...
export default function KPICardsGrid({ analytics }: KPICardsGridProps) {
  ...
  if (!labelsPayload || !labelsPayload.kpi_labels) return null;

  const { kpi_labels, data_confidence_note } = labelsPayload;
  const aiConfidence = kpi_labels["ai_confidence"];

  // Show card for every KPI with value > 0 (tier may be Emerging even for 1–20)
  const activeKpis = Object.entries(kpi_labels)
    .filter(([key, item]) => {
      ...
      // Only hide when value is exactly 0 or null
  ...
  return (
    <div className="space-y-3">
      ...
      {/*
        ...
        flex-1 + min-w-[110px] ensures all cards share equal width
        and the row never wraps on normal dashboard widths.
      */}
        ...
        {activeKpis.map((kpi) => {
          ...
          return (
            ...
            >
              ...
              {/* Row 2: adaptive label — the primary headline */}
              ...
              {/* Row 3: score (bottom-left) + threshold badge (bottom-right) */}
