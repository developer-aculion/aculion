...
# 1. Update log_traffic_overview_history() without peak_traffic_hour
log_history_sql = """
...
BEGIN
    ...
    VALUES (
        ...
        COALESCE(NEW.avg_exposure_time, 0),
        COALESCE(NEW.max_exposure_time, 0),
...
print("Updated log_traffic_overview_history()")

# 2. Update process_traffic_hour() without peak_traffic_hour
...
print("Updated process_traffic_hour()")

# 3. Update process_previous_day() without peak_traffic_hour
process_previous_day_sql = """
...
$function$;
"""
cur.execute(process_previous_day_sql)
print("Updated process_previous_day()")

# 4. Drop peak_traffic_hour columns from all tables
tables_to_drop_col = [
    "traffic_overview",
    ...
    "traffic_month",
    "traffic_year",
    "traffic_hour_backup_20260923"
]

for tbl in tables_to_drop_col:
    cur.execute(f"ALTER TABLE public.{tbl} DROP COLUMN IF EXISTS peak_traffic_hour;")
    print(f"Dropped peak_traffic_hour from {tbl}")
